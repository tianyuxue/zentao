...
RUN apt-get update \
        ...
        && chmod o=rwx -R /var/www/html/tmp/  \
        && chmod o=rwx -R /var/www/html/www/data \
        && docker-php-ext-install pdo_mysql  \
        && cp /usr/local/etc/php/php.ini-production /usr/local/etc/php/php.ini \
        && sed -i "s/;date.timezone =/date.timezone = Asia\/Shanghai/g" /usr/local/etc/php/php.ini \
        && sed -i "s#;session.save_path = \"/tmp\"#session.save_path = \"/var/www/html/tmp/\"#g" /usr/local/etc/php/php.ini
